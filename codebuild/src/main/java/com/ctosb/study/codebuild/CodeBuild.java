...
public class CodeBuild {
	...
	private void initTable() {
		try {
			...
			while (rs.next()) {
				...
				tableModel.addRow(new Object[] { rs.getString("TABLE_NAME"), rs.getString("REMARKS"),
	...
	private void build() throws URISyntaxException {
		...
		String templatePath = Thread.currentThread().getContextClassLoader()
				.getResource(CodeResourceBundleUtil.getTemplatePath()).toURI().getPath();
		// Java文件输出路径
		String outPath = path + "\\" + CodeResourceBundleUtil.getJavaSourceFolder() + "\\"
				+ CodeResourceBundleUtil.getBuildPackage() + "\\" + moduleName;
		// Web文件输出路径
		String jspOutPath = path + "\\" + CodeResourceBundleUtil.getWebSourceFolder() + "\\jsp\\"
				+ CodeResourceBundleUtil.getBuildPackage() + "\\" + moduleName;
		String jsOutPath = path + "\\" + CodeResourceBundleUtil.getWebSourceFolder() + "\\js\\"
				+ CodeResourceBundleUtil.getBuildPackage() + "\\" + moduleName;
		// 模版编译执行
		...
		if (chckbxModel.isSelected()) {
			...
			String fileName = FreemarkerUtil.compile(templatePath, "model.ftl", map,
					outPath + "/model/" + className + ".java");
		...
		if (chckbxDao.isSelected()) {
			log.info("Dao文件开始生成。。。");
			String fileName = FreemarkerUtil.compile(templatePath, "dao.ftl", map,
					outPath + "/dao/" + className + "Dao.java");
			if (fileName == null || fileName.equals("")) {
		...
		if (chckbxService.isSelected()) {
			...
			String fileName = FreemarkerUtil.compile(templatePath, "service.ftl", map,
					outPath + "/service/" + className + "Service.java");
			if (fileName == null || fileName.equals("")) {
			...
			String fileNameImpl = FreemarkerUtil.compile(templatePath, "serviceImpl.ftl", map,
					outPath + "/service/impl/" + className + "ServiceImpl.java");
		...
		if (chckbxController.isSelected()) {
			...
			String fileName = FreemarkerUtil.compile(templatePath, "controller.ftl", map,
					outPath + "/controller/" + className + "Controller.java");
			if (fileName == null || fileName.equals("")) {
		...
		if (chckbxJsp.isSelected()) {
			log.info("Jsp文件开始生成。。。");
			String fileName = FreemarkerUtil.compile(templatePath, "jsp.ftl", map,
					jspOutPath + "\\" + className + ".jsp", true);
			if (fileName == null || fileName.equals("")) {
		...
		if (chckbxJsp.isSelected()) {
			log.info("Js文件开始生成。。。");
			String fileName = FreemarkerUtil.compile(templatePath, "js.ftl", map, jsOutPath + "\\" + className + ".js",
					true);
			if (fileName == null || fileName.equals("")) {
				String msg = "Js文件生成失败。。。";
				sb.append(msg + "\n");
