...
public class TestJdkNewFeature {
    ...
    public void testGenerate() {
        ...
        System.out.println();
        List<User> users = Stream.generate(() -> new User("test", new Random().nextInt(10))).limit(20)
                .collect(Collectors.toList());
        Assert.assertEquals(users.size(), 20);
        Map<Integer, List<User>> map = Stream.generate(() -> new User("test", new Random().nextInt(10))).limit(20)
    ...
    public void testParallel() {
        System.out.println("测试生成构建流");
        List<Integer> list = Stream.generate(() -> new Random().nextInt(1_0000_0000)).limit(1_000_0000)
